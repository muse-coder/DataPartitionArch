...
//    val printWriter = new PrintWriter(new FileWriter(outputFile))
    test(new ScaledFifo(gen = UInt(32.W) ,depth=16)) { dut =>
      //      val dataFromBank = randomNum.nextInt(100)+1
      //      ************************init**********************
//      dut.io.enq.bits.poke(randomNum.nextInt(100))
      dut.io.enq.valid.poke(true.B)
      dut.scaledDepth.poke(10.U)
      //从bank中连续取16个数 存入FIFO  并且前10个cycle store LDR中的值 到bank  后6个cyclestore PE中的值
      var dataIn = 100
      for (i<-1 until 60){
        dut.io.enq.bits.poke(dataIn + i)
        if(i<20){
          dut.io.enq.valid.poke(true.B)
