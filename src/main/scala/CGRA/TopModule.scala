...
class DataPartition [T<:Data] (dataWidth:Int,LSU_InstWidth:Int=65,row:Int=4,column:Int=4,addrWidth:Int=8, PeInstWidth:Int=13,bankNum:Int=8)extends  Module {
  ...
  val Ivg_u = Module(new IVG(addrWidth = addrWidth))
  val LSUs = Array.fill(8){Module(new LSU(dType = UInt(dataWidth.W) ,addrWidth =addrWidth,LSU_InstWidth=LSU_InstWidth,bankNum=bankNum))}
